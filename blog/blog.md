...
und neu zu rendern, allerdings erfordert das auch einen gut strukturierten Datenfluss. Darüber hinaus hat man häufig die Aufgabe, ein und dieselbe Information
...
Im Mittelpunkt des State Management Frameworks von Akita stehen die sogenannten Stores. Ein Store verwaltet die für den Client relevanten Daten und kann
...
an den Store weiterzugeben. Den Komponenten stellt der Service Methoden bereit, mit denen der User Änderungen am Store vornehmen kann. 
...
Akita definiert zusätzlich 4 High-Level Prinzipien, die man als Entwickler beachten sollte. Teilweise ergeben sich die Prinzipien bereits aus dem Akita-Pattern selbst. 

## High-Level Prinzipien

1. Ein Store ist ein einzelnes Objekt, welches den aktuellen Zustand des Stores darstellt und als "Single source of truth" fungiert
...
Wer den kompletten Code nachvollziehen will, findet das Projekt auf [Github](https://github.com/german-reindeer/team-chooser-akita/tree/master/). Ansonsten kann man in dem
Screenshot weiter oben erkennen, wie die Webanwendung aufgebaut ist.  
...
Identifizierung. Um die Vorzüge von Akita voll nutzen zu können, ist es wichtig, dass das Model eine eindeutige ID besitzt. Das Interface ID wird von Akita selbst
bereitgestellt.
...
Vereinfacht kann man sich einen solchen EntityStore als Datenbanktabelle vorstellen. Im Vergleich zu der normalen Store-Klasse von Akita bietet der EntityStore
bereits eigene CRUD-Methoden an, die das Verwalten von Entitäten vereinfachen und uns Boilerplate Code ersparen. Wenn wir vom EntityStore erben, müssen wir sowohl
das Model des verwalteten States (PlayersState), als auch das Model der zu verwaltenden Entität (Player) definieren. Der angegebene State muss wiederum das von Akita
...
dem `players`-Objekt aufrufen, das wir vom Observable erhalten. Einfach ausgedrückt: Wir machen einen REST-Call, der uns alle Spieler einer API zurückgeben soll. Wenn 
wir die Antwort des Calls erhalten, speichern wir die Spieler aus der Antwort in unserem Store. 

Die anderen Methoden bieten weitere Funktionen an, die es für einen CRUD-Service braucht. Die Methoden des Stores, die hier aufgerufen werden, mussten wir 
